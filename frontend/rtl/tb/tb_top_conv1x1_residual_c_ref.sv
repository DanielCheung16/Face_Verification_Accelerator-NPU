...
module tb_top_conv1x1_residual_c_ref;
    ...
    localparam string SRAM_DIR = "gold_models/quantface_conv1x1_c_ref/out_residual";
    ...
    localparam string DUT_AO_FINAL_HEX = {SRAM_DIR, "/quantface_conv1x1_residual_dut_ao_final.hex"};
    ...
    initial begin
        ...
        if (fail_cnt != 0) begin
            $fatal(1, "[TB] top conv1x1 + residual + requant FAILED fail=%0d", fail_cnt);
        end
        $display("[TB] top conv1x1 + residual + requant PASSED");
        $finish;
    end
