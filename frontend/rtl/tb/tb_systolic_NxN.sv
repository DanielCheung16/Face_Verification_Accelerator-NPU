...
module tb_systolic_NxN;
    ...
    logic signed [INTX-1:0]  act_mem [ROW*COL];
    logic signed [INTX-1:0]  wgt_mem [ROW*COL];
    ...
    task automatic drive_systolic_inputs ();
        begin
            for (k_idx = 0; k_idx < ROW + COL + COL; k_idx = k_idx + 1) begin
                @(negedge clk);
                clear_inputs();

                for (x_idx = 0; x_idx < ROW; x_idx = x_idx + 1) begin
                    if ((k_idx >= x_idx) && ((k_idx - x_idx) < COL)) begin
                        row_i[x_idx]       = act_mem[x_idx*COL + (k_idx - x_idx)];
                ...
                for (y_idx = 0; y_idx < COL; y_idx = y_idx + 1) begin
                    if ((k_idx >= y_idx) && ((k_idx - y_idx) < ROW)) begin
    ...
    initial begin
        ...
        $display("[TB] start systolic_NxN test");
        drive_systolic_inputs();

        repeat (ROW + COL + 2) @(posedge clk);
        compare_results();

        if (fail_cnt == 0) begin
            $display("[TB] systolic_NxN PASSED");
            $finish;
        end else begin
            $fatal(1, "[TB] systolic_NxN FAILED");
        end
    end
