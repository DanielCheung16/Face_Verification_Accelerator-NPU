...
\subsection{ASIC Optimization Journey}
\label{sec:opt:asic}
...
\toprule
 & \textbf{v1} & \textbf{v2} & \textbf{v3 (final)} \\
\midrule
Compute        & single 9-MAC dot-product       & dual 9-MAC (18 mult, 2 out-ch parallel) & hybrid SA (14$\times$16 + 3$\times$3 + dw 7$\times$7) \\
Adder          & 38-stage ripple carry          & 40-bit 2-level CLA & CLA + SA pipeline \\
psum memory    & FF array                       & + OpenRAM SRAM macro (psum) & + OpenRAM SRAM macro \\
act memory     & FF array                       & FF array & + OpenRAM SRAM macro (A/O buffer) \\
Cycles / inf   & 43.16\,M                       & 25.77\,M                            & 19.18\,M  \\
...
v1's single 9-MAC unit produced one output channel per cycle, so 8/9 of the available parallelism was wasted on pointwise layers. v2 doubled the MAC count to 18 (two parallel output channels) and replaced the 38-stage ripple-carry adder with a 40-bit two-level CLA. The biggest win was moving the partial-sum memory from an FF array into an OpenRAM SRAM macro: total cell count dropped and post-route power fell from 26.7\,mW to 11.4\,mW even though the MAC count had doubled.

v3 replaced the hand-rolled MAC datapath with the hybrid NPU RTL described in Section~\ref{sec:hw:frontend}: a 14$\times$16 systolic array for $1\times1$ convolution plus a 3$\times$3 standard array and a dw 7$\times$7 engine for the spatial operators. The cycle count drops to 19.18\,M and the clock reaches 200\,MHz. Power rises to 96.3\,mW because the SA fires every cycle at 200\,MHz; the meaningful figure of merit is energy per inference, which lands at 9.24\,$\mu$J.
...
On the FPGA side the dominant optimizations were resource-targeted rather than algorithmic. The 14$\times$16 SA MAC array is mapped onto LUT-based multipliers rather than DSP slices to keep DSP usage low (9.85\%, see Table~\ref{tab:fpga_resource}); BRAM placement is biased toward the WGT RAM, which alone accounts for the bulk of the 54\% BRAM utilisation. Double-buffered window and weight FIFOs (Buf0/Buf1 in the $3\times3$ engine) hide BRAM read latency behind compute and were necessary to hit the 150\,MHz target.
