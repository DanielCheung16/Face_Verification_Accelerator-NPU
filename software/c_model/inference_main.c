    ...
    int is_dw, int has_prelu) 
{
    ...
    sprintf(filepath, "../golden_weights/%s_weight.txt", prefix);
    load_weight_4d(filepath, weight);
    
    sprintf(filepath, "../golden_weights/%s_bias.txt", prefix);
    load_tensor_1d(filepath, bias);
    
    if (has_prelu) {
        sprintf(filepath, "../golden_weights/%s_prelu.txt", prefix);
