...
GOLDEN_WEIGHTS_FIXED_DIR = "../golden_weights_fixed8"
F_BITS = 6  # Q2.6 format (1 sign bit + 1 integer bit + 6 fractional bits)
SCALE_W = (1 << F_BITS)
SCALE_B = (1 << (2 * F_BITS))
# ===========================

def float_to_q8(tensor, scale):
    """ Scales and clamps to INT8 """
    q_tensor = torch.round(tensor * scale)
    q_tensor = torch.clamp(q_tensor, -128, 127)
    return q_tensor.to(torch.int8).numpy().flatten()
...
def export_conv_block_fixed(prefix, conv_module, bn_module, prelu_module):
    ...
    # Quantize
    w_q8 = float_to_q8(w, SCALE_W)
    b_q32 = float_to_q32(b, SCALE_B)  # Bias needs 2*F scale because MAC product is W*X
    
    dump_int_tensor(f"{GOLDEN_WEIGHTS_FIXED_DIR}/{prefix}_weight.txt", w_q8)
    dump_int_tensor(f"{GOLDEN_WEIGHTS_FIXED_DIR}/{prefix}_bias.txt", b_q32)
    
    if prelu_module is not None:
        a_q8 = float_to_q8(prelu_module.weight, SCALE_W)
        dump_int_tensor(f"{GOLDEN_WEIGHTS_FIXED_DIR}/{prefix}_prelu.txt", a_q8)

def main():
    if not os.path.exists(GOLDEN_WEIGHTS_FIXED_DIR):
        os.makedirs(GOLDEN_WEIGHTS_FIXED_DIR)

    print(f"Loading Model for INT8 Quantization (F={F_BITS})...")
    ...
    print(f"Exporting INT8 fixed-point weights to {GOLDEN_WEIGHTS_FIXED_DIR}...")
    ...
    print("INT8 weights exported successfully!")
