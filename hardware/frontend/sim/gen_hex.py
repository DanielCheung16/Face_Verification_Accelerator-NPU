...
# Layer 2: blocks_0_pw1_weight.txt — 1x1 conv (64, 64, 1, 1) = 4096 int16
with open(os.path.join(FIXED_W_DIR, "blocks_0_pw1_weight.txt"), "r") as f:
    pw1_wgt_raw = [int(line.strip()) for line in f]

# For 1x1 PW-Conv, pad each weight with 8 zeros to match the 9-weight ROM format
pw1_wgt = []
for w in pw1_wgt_raw:
    pw1_wgt.append(w)
    pw1_wgt.extend([0] * 8)

all_weights = conv1_wgt + dw1_wgt + pw1_wgt  # concatenated
w_lines = [f"{(w & 0xFFFF):04x}" for w in all_weights]
with open(os.path.join(SIM_DIR, "weights.hex"), "w") as f:
    f.write("\n".join(w_lines))
print(f"Generated weights.hex ({len(w_lines)} entries: conv1={len(conv1_wgt)}, dw1={len(dw1_wgt)}, pw1={len(pw1_wgt)})")

# =============================================================================
# Bias: Conv1 + DW-Conv1 + PW-Conv1
# =============================================================================
...
with open(os.path.join(FIXED_W_DIR, "blocks_0_pw1_bias.txt"), "r") as f:
    pw1_bias = [int(line.strip()) for line in f]

all_biases = conv1_bias + dw1_bias + pw1_bias
b_lines = [f"{(b & 0xFFFFFFFF):08x}" for b in all_biases]
with open(os.path.join(SIM_DIR, "bias.hex"), "w") as f:
    f.write("\n".join(b_lines))
print(f"Generated bias.hex ({len(b_lines)} entries: conv1={len(conv1_bias)}, dw1={len(dw1_bias)}, pw1={len(pw1_bias)})")

# =============================================================================
# PReLU: Conv1 + DW-Conv1 + PW-Conv1
# =============================================================================
...
with open(os.path.join(FIXED_W_DIR, "blocks_0_pw1_prelu.txt"), "r") as f:
    pw1_prelu = [int(line.strip()) for line in f]

all_prelus = conv1_prelu + dw1_prelu + pw1_prelu
p_lines = [f"{(p & 0xFFFF):04x}" for p in all_prelus]
with open(os.path.join(SIM_DIR, "prelu.hex"), "w") as f:
    f.write("\n".join(p_lines))
print(f"Generated prelu.hex ({len(p_lines)} entries: conv1={len(conv1_prelu)}, dw1={len(dw1_prelu)}, pw1={len(pw1_prelu)})")
...
# =============================================================================
def make_config(in_ch, out_ch, w, h, stride, has_prelu, ping_pong, is_pw, is_dw, wgt_base):
    val = 0
    val |= (in_ch & 0x3FF)
    val |= (out_ch & 0x3FF) << 10
    val |= (w & 0xFF) << 20
...
# Layer 0: Conv1 — stride=2, h=112, w=96, out=64, in=3, prelu=1, pp=0, pw=0, dw=0, wgt_base=0
config0 = make_config(in_ch=3, out_ch=64, w=96, h=112, stride=2,
                      has_prelu=1, ping_pong=0, is_pw=0, is_dw=0, wgt_base=0)

# Layer 1: DW-Conv1 — stride=1, h=56, w=48, out=64, in=64, prelu=1, pp=1, pw=0, dw=1, wgt_base=1728
w_base_1 = len(conv1_wgt)
config1 = make_config(in_ch=64, out_ch=64, w=48, h=56, stride=1,
                      has_prelu=1, ping_pong=1, is_pw=0, is_dw=1, wgt_base=w_base_1)

# Layer 2: PW-Conv1 — stride=1, h=56, w=48, out=128, in=64, prelu=1, pp=0, pw=1, dw=0
w_base_2 = w_base_1 + len(dw1_wgt)
config2 = make_config(in_ch=64, out_ch=128, w=48, h=56, stride=1,
                      has_prelu=1, ping_pong=0, is_pw=1, is_dw=0, wgt_base=w_base_2)

with open(os.path.join(SIM_DIR, "config.hex"), "w") as f:
    f.write(f"{config0:016x}\n")
    f.write(f"{config1:016x}\n")
    f.write(f"{config2:016x}\n")
    for _ in range(61):
        f.write("0000000000000000\n")

print(f"Generated config.hex:")
print(f"  Layer 0: Conv1   — h=112, w=96, stride=2, in=3, out=64, wgt_base=0")
print(f"  Layer 1: DW-Conv — h=56, w=48, stride=1, in=64, out=64, wgt_base={w_base_1}, is_dw=1, pp=1")
print(f"  Layer 2: PW-Conv — h=56, w=48, stride=1, in=64, out=128, wgt_base={w_base_2}, is_pw=1, pp=0")
